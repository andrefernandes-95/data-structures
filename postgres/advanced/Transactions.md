...
- Our bank's officers will want to be assured that either all these updates happen, or none of them happen. It would certainly not do for a system failure to result in Bob receiving $100.00 that was not debited from Alice. Nor would Alice long remain a happy customer if she was debited without Bob being credited.
...
Prevents other users from seeing intermediate or partial updates.

| Step | Transaction 1 (T1)        | Transaction 2 (T2) |
| ---- | ------------------------- | ------------------ |
| 1    | Update balance: 100 → 150 | —                  |
| 2    | —                         | Read balance       |
| 3    | Commit                    | —                  |


- If T2 reads before T1 commits, it will see the old balance (100).

- After T1 commits, T2 (or any new transaction) sees the updated balance (150).

This prevents dirty reads, where a transaction sees changes that might later be rolled back.

💡 Analogy

> Think of editing a shared Google Doc offline:
> - You make changes in your copy (invisible to others).
> - Others continue seeing the old version.
> - When you “publish” your changes, everyone sees your edits at once.
